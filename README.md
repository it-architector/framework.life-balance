...
Среда в которой мы находимся может как помогать нам эволюционировать, так и наоборот. Я к сожалению, опиравшись на популярные framework,  подножек и ловушек хорошо понаполучал. И понял, что текущая система работает против новичков.

![](/Компоненты%20интерфейса/3.Распределение/Элементы/Элементы%20картинок/illustrators/Тупая%20пила.jpg)

Это мне не понравилось, и я решил со всех реализованных framework сбросить лишнее, дабы по остаткам выявить нужное и собрать из них Framework Life Balance.
...
Структура проекта проста: должна быть изоляция ядра (back-end) от интерфейса (front-end). Взаимодействие между ними утверждено в виде "запрос-ответ".

Для минимизации ручного труда были утверждены облегчающие алгоритмы: фиксация сбоя, фоновый запуск, работа с базой данных массивами (в том числе авто-создание-изменение таблиц с полями), чпу файлами.
...
p.s.2. Данный framework (как и другие) всё так же беструктурен, но дал шанс родиться структуре <a target="_blank" href="https://github.com/it-architector/structure.csdr">csdr</a>.
