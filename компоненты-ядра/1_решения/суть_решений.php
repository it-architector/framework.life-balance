...
class Solutions implements Structure_Solutions
{
    ...
    static function initiation(){
        ...
        $config_system = Resources::include_information_from_file(DIR_SOLUTIONS,'настройка_системы','php');
        ...
        $include_module_structure_data_base = Resources::include_information_from_file(DIR_STRUCTURE_THEIR_MODULES,'модуль_базы_данных_' . $config_system['data_base'],'php');

        if($include_module_structure_data_base === null){
            Business::fix_error('нет структуры модуля работы с базой данных ' . $config_system['data_base'],__FILE__, __LINE__);
        }

        $include_module_data_base = Resources::include_information_from_file(DIR_THEIR_MODULES,'модуль_базы_данных_' . $config_system['data_base'],'php');

        if($include_module_data_base === null){
            Business::fix_error('нет модуля работы с базой данных ' . $config_system['data_base'],__FILE__, __LINE__);
        }
    ...
    static function construct_class_experience($experience){
        ...
        if (!interface_exists($experience_structure_name)) {
            ...
            Resources::include_information_from_file(DIR_STRUCTURE_EXPERIENCES, 'наработка_'.$experience,'php');
        }
        ...
        if (!class_exists($experience_class_name)) {
            /*подключаем файл с классом наработки*/
            Resources::include_information_from_file(DIR_EXPERIENCES, 'наработка_'.$experience,'php');
        }
